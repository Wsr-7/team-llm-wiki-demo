...
for (const file of listFiles("wiki", [".md"])) {
  ...
  if (!type || !allowedTypes.has(type)) {
    errors.push(`${path}: invalid type ${String(data.type)}`);
  } else if (!path.startsWith(typeDirectories[type])) {
    errors.push(`${path}: type ${type} must live under ${typeDirectories[type]}`);
  }

  const status = getString(data, "status");
  if (!status || !allowedStatus.has(status)) {
    errors.push(`${path}: invalid status ${String(data.status)}`);
  }

  const reviewState = getString(data, "review_state");
  if (!reviewState || !allowedReviewState.has(reviewState)) {
    errors.push(`${path}: invalid review_state ${String(data.review_state)}`);
  }

  const visibility = getString(data, "visibility");
  if (!visibility || !allowedVisibility.has(visibility)) {
    errors.push(`${path}: invalid visibility ${String(data.visibility)}`);
  }
  ...
  if (owners.length === 0) {
    errors.push(`${path}: owners must contain at least one staff id`);
  }
  for (const owner of owners) {
    if (!/^staff:[0-9]{8}$/.test(owner)) {
      errors.push(`${path}: invalid owner ${owner}`);
    }
  }

  const sourceRefs = getArray(data, "source_refs");
  if (sourceRefs.length === 0) {
    errors.push(`${path}: source_refs must contain at least one source`);
  }

  if (type === "mirrored") {
    for (const field of ["mirror_provider", "remote_id", "remote_url", "remote_version", "remote_hash", "synced_at", "collector", "mirror_status"]) {
      if (!(field in data)) {
        errors.push(`${path}: mirrored pages must include ${field}`);
      }
    }
  }
}
