...
export const WIKI_SECTIONS = [
  { dir: "troubleshooting", label: "Troubleshooting — 生产问题排障" },
  { dir: "runbooks", label: "Runbooks — 操作手册" },
  { dir: "systems", label: "Systems — 系统" },
  { dir: "decisions", label: "Decisions — 决策记录" },
  { dir: "concepts", label: "Concepts — 领域概念" },
  { dir: "guides", label: "Guides — 指南与实践" },
];
...
export function buildIndexContent(pages) {
  const lines = [
    "# INDEX",
    "",
    "> 由 `npm run build-index` 生成，请勿手工编辑。每页一行：标题 — 摘要。",
    "",
  ];
  const entryFor = (page) => {
  ...
  lines.push("- [人员路由表](team/people.md) — staff-id ↔ GitHub ↔ 负责域", "");
  return lines.join("\n");
}
